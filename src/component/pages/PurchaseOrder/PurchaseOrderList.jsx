...
function PurchaseOrderList({ permission }) {
  ...
  const fetchOrderList = async (search) => {
    try {
      ...
      const response = await axiosInstance.get(
        `${AxiosInstancePaths.PurchseOrders.GET_LIST}?${queryString}`
      );
  ...
  const editOrder = (id) => {
    navigate(`/purchase_orders/edit/${id}`);
  };
  const viewOrder = (id) => {
    navigate(`/purchase_orders/view/${id}`);
  };

  const handleFilter = (name, value) => {
    if (name === "billing_amount" || name === "invoice_no" || name === "advance_payment_amount") {
      setFilter({
        sortBy: value,
        sortField: name,
      });
  ...
  const downloadExcel = async () => {
    try {
      ...
      const response = await axiosInstance.get(AxiosInstancePaths.PurchseOrders.DOWNLOAD_EXCEL, {
      ...
      saveAs(blob, 'purchse_orders.xlsx');
  ...
  return (
    <div>
      <TableHeaderPart
        ...
        searchText="Invoice No, Status"
        handleAdd={() => navigate("/purchase_orders/add")}
        ...
        <div style={{
          ...
          <div style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "center" }}>
            ...
            <DropDrown
              ...
              icon={
                filter?.sortField &&
                  filter.sortField === "advance_payment_amount" &&
                  filter.sortBy === "asc" ? (
